...
export default function Navigation() {
  ...
  // Close on Escape — scoped to when menu is open
  ...
  return (
    <header
      className={`sticky top-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-white/[0.98] backdrop-blur-md shadow-[0_1px_0_rgba(30,43,56,0.08)]'
          : 'bg-surface/95 backdrop-blur-sm border-b border-surface-muted'
      }`}
    >
      ...
      <div className="section-inner">
        <div className="flex items-center justify-between h-[4.25rem]">
          ...
          <Link
            ...
            aria-label="Leadership Infrastructure Systems — home"
          >
            ...
            <div className="flex flex-col leading-none">
              <span className="font-display font-semibold text-navy text-[1.0625rem] tracking-[-0.01em]">
                LIS
              </span>
              <span className="font-body font-medium text-ink-soft text-[0.5625rem] tracking-[0.13em] uppercase mt-[3px]">
          ...
          <div className="hidden lg:flex items-center gap-5">
            ...
            <Link
              href="/eli-snapshot"
              className="btn-eli text-[0.75rem] tracking-[0.07em] px-5 py-2.5"
            >
              Take ELI Snapshot
            </Link>
          </div>
          ...
          >
            <span
              className={`block h-[1.5px] w-5 bg-navy transition-all duration-200 origin-center ${
                open ? 'translate-y-[6.5px] rotate-45' : ''
              }`}
            />
            <span
              className={`block h-[1.5px] w-5 bg-navy transition-opacity duration-200 ${
                open ? 'opacity-0' : ''
              }`}
            />
            <span
      ...
      >
        <div className="overflow-hidden">
          <div className="bg-white border-t border-surface-muted">
            <nav
              className="section-inner py-4 flex flex-col divide-y divide-surface-muted"
              aria-label="Mobile"
            >
              {navLinks.map(link => (
                <Link
              ...
              <div className="pt-5 pb-2">
                <Link
                  href="/eli-snapshot"
                  tabIndex={open ? 0 : -1}
                  className="btn-eli w-full justify-center"
                >
